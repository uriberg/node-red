...
var RED = (function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                for (let i = 0; i < data.length; i++){
                    if (data[i].name === "energy-minimization" || data[i].name === "assign-hydrogens" || data[i].name === "ligand-file-conversion" || data[i].name === "protein-preparation" || data[i].name === 'ligand-preparation' || data[i].name === 'pymol-fetch' || data[i].name === 'docking' || data[i].name === 'molecular-dynamics' || data[i].name === 'rcsb-fetch'){
                        newData.push(data[i]);
                        console.log(newData);
                    }
                }
                RED.nodes.setNodeList(newData);
    ...
    function completeLoad() {
        ...
        $(".fa-warning").hide();
        $("#red-ui-header").css({'display': 'flex', 'align-items': 'center','justify-content': 'space-between', 'background': '#f3f3f3'});
        $(".red-ui-header-logo").css({'margin-top': '0'});
        $(".red-ui-header-logo img").css({'height': '40px'});
        $(".red-ui-header-logo span").css({'color': 'black'});
        $(".red-ui-editor .form-row").css({'display': 'flex', 'align-items': 'center'});
        const $elem = $(".red-ui-palette-node");
        for(let i =0; i < $elem.length; i++){
            $elem[i].style.setProperty('height', 'auto', 'important');
        }
        ...
        $.getJSON(options.apiRootUrl+"theme", function(theme) {
            if (theme.header) {
                ...
                if (theme.header.image) {
                    $('<img>',{src:'https://media-exp1.licdn.com/dms/image/C560BAQG6aSECYlRyKA/company-logo_200_200/0/1562609499791?e=2159024400&v=beta&t=T8Sz1KnGf8HRbPwR7qt9i0w0UDG3U1MAhCJdK5zfHvo'}).appendTo(logo);
                }
                if (theme.header.title) {
                    // $('<span>').html(theme.header.title).appendTo(logo);
                    $('<span>').html('Welcome to Interx').appendTo(logo);
                }
            }
            if (theme.themes) {
